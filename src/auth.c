...
typedef enum {IDENTITY=1, NOTIFICATION=2, MD5=4, ALLOCATED=7, AVAILABLE=20} EAP_Type; //增加EAP_Type=7
...
const char H3C_VERSION[16]="EN V2.40-0335"; // 华为客户端版本号EN V2.40-0335
...
static void SendResponseIdentity(pcap_t *adhandle,
			...
			const uint8_t ip[4],
			const char    username[]);
static void SendResponseIdentityUserName(pcap_t *adhandle,
			const uint8_t request[],
			const uint8_t ethhdr[],
			const uint8_t ip[4],
			const char    username[]);
...
int Authentication(const char *UserName, const char *Password, const char *DeviceName)
{
	...
	START_AUTHENTICATION:
	{
		...
		{	// 通常情况会收到包Request Identity，应回答Response Identity
			...
			SendResponseIdentityUserName(adhandle, captured, ethhdr, ip, UserName);
			DPRINTF("[%d] Client: Response Identity-UserName:[%s]!\n", (EAP_ID)captured[19],UserName);//增加打印用户名
		}
...
static
void SendResponseIdentityUserName(pcap_t *adhandle, const uint8_t request[], const uint8_t ethhdr[], const uint8_t ip[4], const char username[])
{
	uint8_t	response[128];
	size_t i;
	uint16_t eaplen;
...
static
void SendResponsePassword(pcap_t *handle, const uint8_t request[], const uint8_t ethhdr[], const char username[], const char passwd[]) //SendResponsePassword(adhandle, captured, ethhdr, UserName, Password);
{
	uint16_t eaplen;
	size_t   usernamelen;
	size_t   passwordlen;
